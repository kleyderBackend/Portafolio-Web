# Portafolio Web Interactivo

Portafolio personal de Kleyder Moreno Dávila desarrollado con HTML5, CSS3 y JavaScript vanilla. El sitio presenta perfil profesional, stack tecnológico, proyectos destacados y un formulario de contacto simulado con validaciones.

## Tecnologías utilizadas

- HTML5 semántico
- CSS3 con Flexbox, Grid, variables CSS, transiciones y media queries
- JavaScript vanilla para menú responsive, renderizado dinámico de proyectos y validación del formulario
- Git y GitHub Pages para control de versiones y despliegue

## Estructura del proyecto

```text
Portafolio-Web/
├── index.html
├── aboutme.html
├── projects.html
├── contacts.html
├── css/
│   ├── variables.css
│   ├── style.css
│   ├── aboutme.css
│   ├── project.css
│   └── contacts.css
├── js/
│   ├── app.js
│   ├── projects.js
│   └── contacts.js
├── img/
├── cv/
└── docs/
    └── entregables-aeae2.md
```

## Ejecución local

Abre `index.html` en el navegador. Al ser un sitio estático, no requiere instalación de dependencias ni servidor backend.

## Funcionalidades

- Navegación entre secciones del portafolio.
- Menú hamburguesa para pantallas móviles.
- Lista de proyectos renderizada desde un array de objetos en JavaScript.
- Formulario de contacto con validación HTML5 y validación en tiempo real con JavaScript.
- Diseño responsivo para móvil, tablet y escritorio.

## Capturas de pantalla

Las capturas deben tomarse desde el navegador después de publicar o abrir el sitio localmente. Vistas recomendadas para el entregable:

- Inicio en escritorio.
- Proyectos en escritorio.
- Contacto en móvil con menú hamburguesa.
- Validación del formulario de contacto.

## Autor

Kleyder Moreno Dávila.

## Licencia

Proyecto académico de uso educativo.
